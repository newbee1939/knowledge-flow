...
flowchart TD
    subgraph old["旧: 名札方式"]
      direction TB
      A[AI] -- 名札IDで<br/>接続維持 --> B[特定の1台に固定]
    end
    subgraph new["新: 使い切り方式"]
      direction TB
      C[AI] -- 毎回情報を同梱 --> D[どのサーバーでもOK]
      D --> E[負荷分散<br/>サーバーレス対応]
    end
    old --> new
```
...
```mermaid
flowchart TD
    subgraph fixed["従来: 箱ごと固定"]
      X[CPU+メモリ+GPU が一体]
    end
    subgraph cohdi["CoHDI: 部品プールから組立"]
...
```mermaid
flowchart TD
    A[MVCCの代償は<br/>誰かが必ず払う] --> B[PostgreSQL<br/>後の掃除]
    A --> C[Oracle/InnoDB<br/>履歴の読み手]
    A --> D[SQL Server<br/>共有領域]
    A --> E[LSM系<br/>圧縮処理]
```
...
```mermaid
flowchart TD
    A[初期侵入を防ぐ] --> A1[アカウント変更後<br/>72時間は制限]
    A --> A2[Actionsで<br/>外部コードを禁止]
    A1 --> B[認証情報を守る]
    A2 --> B
    B --> B1[長期間有効な<br/>鍵を廃止]
    B1 --> C[拡散を止める]
    C --> C1[インストール時<br/>スクリプトを無効]
    C --> C2[Dependabotに<br/>3日間の待機期間]
```
