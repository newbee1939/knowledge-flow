# knowledge-flow — 開発ルール

## コミュニケーション

- **ユーザーからの質問には、なるべくシンプルかつ分かりやすく回答する。** 前提知識を過度に要求せず、要点から簡潔に答える

## 実装方針

- 最小実装を選ぶ。痛みを感じてから機能・抽象化を追加する
- シンプルで読みやすく、保守しやすいコードを書く
- **`let` はなるべく使わず `const` で書く。** 再代入が必要に見える処理も、先に `reduce` / `map` / `find` などの配列メソッドで再代入不要な形に書けないか検討する。`for` ループも同様に、配列メソッドで置き換えられないか先に考える
- **他ファイルから参照されない型・関数は `export` しない。** モジュール内部でしか使わないものまで公開すると、変更していい範囲が外から見えにくくなる

## Node のバージョン

- **`.tool-versions` が唯一の真実。** ローカルは **mise**、CI は `setup-node` が、**どちらもこのファイルをそのまま読む**
- `.nvmrc` は使わない。**mise は `.nvmrc` を既定で読まない**（idiomatic version files は既定で無効）ため、置いても無視され、ローカルと CI で Node がズレる
- バージョンを上げるときは `.tool-versions` を1行直すだけ。ローカルは `mise install` で追従する

## テスト

- **ロジックを書いたらテストも書く。** 対象は `src/lib/` などの純粋な関数・データ変換
- 置き場所は**テスト対象と同じ階層に `*.test.ts`**（`src/lib/url.ts` → `src/lib/url.test.ts`）。専用の `tests/` ディレクトリは作らない
- テストランナーは **Vitest**（`npm test`）。CI で必ず回る
- **テストを書かないもの**: `.astro` のテンプレート（見た目は目で見る。P2-10 のレスポンシブ確認で担保）、外部 API を叩くだけのコード
- **「壊れたら気づけない」ものを優先してテストする。** カバレッジの数字は追わない

## リファクタリング

- **機会があれば、より可読性・保守性の高いシンプルな形へ繰り返しリファクタリングする。** 「動いている」で満足せず、状態や分岐を減らせる構造がないか定期的に見直す
- **先に挙動を固定するユニットテストを書いてから着手する**（仕様化テスト）。テストが無いコードはテストを足すのが先
- **リファクタ後もテストを1文字も変えずに全件通す**ことを「挙動不変」の証拠とする。テストを書き換えないと通らないなら、それはリファクタではなく仕様変更
- 小さく刻む。1回で1つの構造だけ変え、都度 `npm run lint && npm test` を回す
- 良し悪しの目安: 持ち回る状態（state のフィールド、フラグ）が減るか／人に説明するのに必要な文章量が減るか

## Lint / Format

- **Biome**（`npm run lint` / `npm run lint:fix`）。ESLint + Prettier は使わない（依存を増やさない）
- CI で `npm run lint` が回る。フォーマット差分があれば落ちる

## GitHub Actions

- **Action はタグではなくコミットハッシュで固定する。** タグは書き換え可能なので、リポジトリが乗っ取られれば `@v7` が別のコミットを指しうる（サプライチェーン攻撃）
- 末尾に `# v7.0.0` のようにバージョンをコメントで残す。Dependabot はこれを見て更新 PR を出す

```yaml
- uses: actions/checkout@9c091bb21b7c1c1d1991bb908d89e4e9dddfe3e0 # v7.0.0
```

- ワークフローで**信用できない入力**（issue タイトル、PR 本文、コミットメッセージ等）を `run:` に直接埋め込まない。`env:` 経由で渡す（コマンドインジェクション対策）

## ブランチ・PR ワークフロー

- 基本的には**1 タスク（TASK.md 1項目）= 1 ブランチ = 1 PR**
    - 必要に応じて複数タスクを1つのPRにまとめるのは許可
- 以下を実行してからタスクを開始する

```
# ブランチ命名: feature/任意の命名
git co main && git pull origin main
git co -b feature/任意の命名
```

コミットメッセージ: `<type>: <概要> (<task-id>)`
type: `feat` / `fix` / `chore` / `docs` / `style` / `refactor` / `ci`

- PR上でAIによるレビューが来るので対応する
    - 修正したら、それぞれのコメントに返信した上でコメントをResolveする
- **実装を追加・修正したら、PR の description も必ず最新化する。** 差分と description がズレると、レビュアーが古い前提で読んでしまう
- 必ず出ているPRをマージしてから次のタスクに進む

## コミット前のセルフレビュー

- **`git diff` を批判的な目で読み直してから commit する。** 「動いた」で終わらせず、バグ・エッジケースの考慮漏れ・過剰な複雑化・紛らわしい命名がないか疑う
- 1 コミット単位でも省略しない。PR 全体の確認は次項「セルフレビュー（PR 前に確認）」で別途行う

## セルフレビュー（PR 前に確認）

- [ ] TASK.md の DoD をすべて満たしているか
- [ ] TASK.md のチェックボックスを `- [x]` に更新したか
- [ ] 最小実装か（余計な機能がないか）
- [ ] ロジックを足したなら、隣に `*.test.ts` を書いたか
- [ ] `npm run lint && npm test && npm run build` がローカルで通るか

## 敵対的検証（影響が大きい変更のレビュー）

出典: [AIエージェントに「敵対的検証」させる](https://zenn.dev/loglass/articles/6aa18c80496ec6)

**対象**: 認証・権限・公開範囲・GitHub Actions のワークフローなど、間違えたときの被害が大きい変更。すべての PR に要求すると重すぎるので、リスクの大きいものに絞る

- **作成した文脈から切り離した新規セッションでレビューさせる。** 同じセッションの続きだと「自分が正しいと思っている前提」を引きずる
- 指示は「レビューして」ではなく**「反証を試みよ」**と懐疑的に出す。間違っている可能性を前提に検討させる
- 実際のコード・diff・公式ドキュメントなど**一次情報に基づいて検証させる**。記憶や推測に頼らせない
- 指摘には**深刻度と根拠**を付けさせ、人間が採否を判断できる形にする
- **AI の指摘は過剰になりがち。** 採否の最終判断は人間の責任として鵜呑みにしない
